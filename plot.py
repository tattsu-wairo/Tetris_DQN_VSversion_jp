<<<<<<< HEAD
import matplotlib.pyplot as plt
import numpy as np
import os
import argparse

def initialize_plot():
    plt.figure(figsize=(10, 5))
    plt.title('Tetris-DQN')
    plt.xlabel('total games')
    plt.ylabel('rewards')

def DQN():
    DQN_Rewards = np.load("./Rewards/DQN_rewards.npy")

    DQN_avg = np.mean(DQN_Rewards, axis=1)
    DQN_std = np.std(DQN_Rewards, axis=1)

    initialize_plot()

    plt.plot([i * 25 for i in range(200)], DQN_avg,
             label='DQN', color='blue')
    plt.fill_between([i * 25 for i in range(200)],
                     DQN_avg+DQN_std, DQN_avg-DQN_std, facecolor='lightblue')
    plt.legend(loc="best")
    plt.savefig("./Graphs/DQN.png")
    plt.show()
    plt.close()

def DQN_Hold():
    #tmp_table = np.load("./Rewards/DQN_rewards.npy").transpose()
    DQN_Rewards = np.load("./Rewards/DQN_rewards_hold.npy")

    DQN_avg = np.mean(DQN_Rewards, axis=1)
    DQN_std = np.std(DQN_Rewards, axis=1)

    initialize_plot()

    plt.plot([i * 25 for i in range(200)], DQN_avg,
             label='DQN_Hold', color='orange')
    plt.fill_between([i * 25 for i in range(200)],
                     DQN_avg+DQN_std, DQN_avg-DQN_std, facecolor='lightblue')
    plt.legend(loc="best")
    plt.savefig("./Graphs/DQN_hold.png")
    plt.show()
    plt.close()

def compare():
    DQN_Rewards = np.load("./Rewards/DQN_rewards.npy")
    DQN_avg = np.mean(DQN_Rewards, axis=1)
    Hold_DQN_Rewards = np.load("./Rewards/DQN_rewards_hold.npy")
    Hold_DQN_avg = np.mean(Hold_DQN_Rewards, axis=1)
    initialize_plot()
    plt.plot([i * 25 for i in range(200)], DQN_avg, label='DQN', color='blue')
    plt.plot([i * 25 for i in range(200)],
             Hold_DQN_avg, label='DQN_Hold', color='orange')
    plt.legend(loc="best")
    plt.savefig("./Graphs/compare.png")
    plt.show()
    plt.close()

if __name__ == "__main__":
    '''
    Plot the trend of Rewards
    '''
    parser = argparse.ArgumentParser()
    parser.add_argument("--DQN", action="store_true")
    parser.add_argument("--DQN_Hold", action="store_true")
    parser.add_argument("--compare", action="store_true")
    args = parser.parse_args()

        
    if not os.path.exists("./Graphs"):
        os.mkdir("./Graphs")

    if args.DQN:
        DQN()
    elif args.DQN_Hold:
        DQN_Hold()
    elif args.compare:
=======
import matplotlib.pyplot as plt
import numpy as np
import os
import argparse

def initialize_plot():
    plt.figure(figsize=(10, 5))
    plt.title('Tetris-DQN')
    plt.xlabel('total games')
    plt.ylabel('rewards')

def DQN():
    DQN_Rewards = np.load("./Rewards/DQN_rewards.npy")

    DQN_avg = np.mean(DQN_Rewards, axis=1)
    DQN_std = np.std(DQN_Rewards, axis=1)

    initialize_plot()

    plt.plot([i * 25 for i in range(200)], DQN_avg,
             label='DQN', color='blue')
    plt.fill_between([i * 25 for i in range(200)],
                     DQN_avg+DQN_std, DQN_avg-DQN_std, facecolor='lightblue')
    plt.legend(loc="best")
    plt.savefig("./Graphs/DQN.png")
    plt.show()
    plt.close()

def DQN_Hold():
    #tmp_table = np.load("./Rewards/DQN_rewards.npy").transpose()
    DQN_Rewards = np.load("./Rewards/DQN_rewards_hold.npy")

    DQN_avg = np.mean(DQN_Rewards, axis=1)
    DQN_std = np.std(DQN_Rewards, axis=1)

    initialize_plot()

    plt.plot([i * 25 for i in range(200)], DQN_avg,
             label='DQN_Hold', color='orange')
    plt.fill_between([i * 25 for i in range(200)],
                     DQN_avg+DQN_std, DQN_avg-DQN_std, facecolor='lightblue')
    plt.legend(loc="best")
    plt.savefig("./Graphs/DQN_hold.png")
    plt.show()
    plt.close()

def compare():
    DQN_Rewards = np.load("./Rewards/DQN_rewards.npy")
    DQN_avg = np.mean(DQN_Rewards, axis=1)
    Hold_DQN_Rewards = np.load("./Rewards/DQN_rewards_hold.npy")
    Hold_DQN_avg = np.mean(Hold_DQN_Rewards, axis=1)
    initialize_plot()
    plt.plot([i * 25 for i in range(200)], DQN_avg, label='DQN', color='blue')
    plt.plot([i * 25 for i in range(200)],
             Hold_DQN_avg, label='DQN_Hold', color='orange')
    plt.legend(loc="best")
    plt.savefig("./Graphs/compare.png")
    plt.show()
    plt.close()

if __name__ == "__main__":
    '''
    Plot the trend of Rewards
    '''
    parser = argparse.ArgumentParser()
    parser.add_argument("--DQN", action="store_true")
    parser.add_argument("--DQN_Hold", action="store_true")
    parser.add_argument("--compare", action="store_true")
    args = parser.parse_args()

        
    if not os.path.exists("./Graphs"):
        os.mkdir("./Graphs")

    if args.DQN:
        DQN()
    elif args.DQN_Hold:
        DQN_Hold()
    elif args.compare:
>>>>>>> 768856545e71a33ed88b61439bfdae59110fe7c0
        compare()